"""Cross-platform VideoLingo Worker: faster-whisper STT + Ollama translation."""
import asyncio, json, os, platform, shutil, tempfile, uuid
...
async def status(authorization: str | None = Header(default=None)):
    auth(authorization)
    return {"apiVersion": 1, "workerID": str(WORKER_ID), "name": NAME,
            "version": f"1.1 ({platform.system()} {platform.machine()})",
            ...
            "capabilities": {"sttSlots": STT_SLOTS, "translationSlots": TRANSLATION_SLOTS,
                             "sttModels": [f"faster-whisper/{WHISPER_MODEL}"],
                             "translationModels": [f"ollama/{OLLAMA_MODEL}"]}}
