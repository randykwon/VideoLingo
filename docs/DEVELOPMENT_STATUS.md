...
- 창별 `번역 모드`와 `영상 감상 모드` 전환 및 마지막 선택 저장
- 번역 모드에서는 STT·번역·품질·결과 관리 중심의 전체 사이드바 제공
- 영상 감상 모드에서는 AI 설정을 감추고 영상 열기·재생·탐색·음량·자막·화면 메뉴에 집중
- 감상 모드의 음량 슬라이더, 현재/전체 시간, 자막 표시·언어·이중 자막 제어
- 감상 모드 전환 중에도 실행 중인 STT·번역 작업을 백그라운드에서 유지
- 감상 모드에서는 결과 편집 패널을 감춰 플레이어 공간 확대
- `AVPlayer` 기반 MP4 재생
...
- Vision 기반 재생 화면 OCR
- 화면에서 인식한 텍스트의 실시간 번역 자막 표시(성능 보호 및 재설계를 위해 임시 비활성화)
...
- 대량 번역 툴바에서 모든 동일 이름 그룹을 한 번에 검사하고 그룹별 유지본 1개를 제외한 영상을 목록에서 일괄 제거
- 일괄 정리 확인 화면에서 `실제 영상 파일도 휴지통으로 이동`을 체크하면 그룹마다 실제 파일 1개만 남기고 나머지를 휴지통으로 이동
- 일괄 제거 전 대상 개수와 실제 영상·STT 결과가 삭제되지 않음을 확인하며, 경로별 검토 화면에서 중복 전체 선택·해제 지원
- 그룹마다 첫 영상을 유지 대상으로 두고 나머지를 목록 제외 대상으로 기본 선택
- 검토 선택을 바꿔 여러 중복 영상을 한 번에 대량 번역 목록에서 제거
- 기본 중복 정리는 실제 영상 파일과 저장된 STT·번역 결과를 삭제하지 않음
- 동일 이름 검토 창에서 명시적으로 선택한 실제 영상 파일을 macOS 휴지통으로 이동
- 휴지통 이동 전 대상 개수·영향을 재확인하고 실행 중에는 기능 잠금
- 일반 파일 여부와 심볼릭 링크를 검증하며 실패 시 즉시 중단하고 경로·원인 표시
- 휴지통 이동 성공 항목만 대량 번역 목록에서 제거하고 STT·번역 DB 결과는 유지
- 숨김 파일, 앱 패키지 내부, 심볼릭 링크 건너뛰기
- 드래그 중 드롭 영역 강조와 지원 형식 안내
- 영상이 아닌 파일 및 이미 추가된 파일 자동 제외
- 메인 창의 현재 STT 모델, 번역 LLM, 원어, 대상 언어, 품질 설정 사용
- 대량 번역 창에서 STT 원어 자동 감지·직접 지정과 복수 번역 언어를 한곳에서 설정
- 파일 상세 진행 창에서도 같은 배치 언어 설정을 확인·변경하고 모든 창에 즉시 동기화
- 언어 변경 시 새 설정 기준으로 기존 STT·번역 결과 자동 재검사
- 실행 중에는 작업 식별자와 결과 일관성을 위해 언어 변경 잠금 및 사유 표시
...
- 별도 `멀티 화면 모니터` 창에서 여러 영상을 음소거로 동시에 재생하며 처리 상태를 시각적으로 확인
- 멀티 화면 카드마다 STT·번역 독립 진행률, 현재 청크, 실시간 원문·번역문을 함께 표시
- 실행 중·전체·완료 필터, 1~4열 화면 구성 저장 및 상세 진행 창 바로가기
- 멀티 화면 영상은 기본적으로 재생하지 않으며 카드별 체크박스로 선택한 영상만 음소거 재생
- 현재 필터에 표시된 영상의 재생 전체 선택·해제와 선택 개수 표시
- 동시 처리 수 1~10개 설정 및 사용자 선택 저장
- 기본 동시 처리 수 5개
- 기본 자동 모드에서 Mac의 물리 메모리·CPU 코어·열 상태에 맞춰 동시 처리 수를 1~5개로 결정
- 열 상태가 높아지면 다음 실행의 동시 처리 수를 최대 2개 또는 1개로 제한하며 필요하면 수동 1~10개 설정 사용
- 성능 기능 재설계 전까지 STT·번역 상태 확인 주기를 2초로 낮추고 화면 글자 OCR·번역 기능 임시 비활성화
- 대량 번역 또는 선택 항목 시작 전에 STT 원어, 번역 대상 언어, STT·번역 모델, 동시 처리 수와 실행 대상 개수를 확인하는 사전 점검 화면
- 사전 점검 화면에서 원어·번역 언어와 동시 처리 수를 바로 수정하고 기존 결과 재검사가 끝난 뒤 확인하여 시작
- 파일별 상태, 진행률, 현재 작업 메시지 표시
- 대량 번역 목록을 `전체`, `진행·대기`, `번역 완료`로 분리해 개수와 함께 필터링
- 번역 완료 목록에서 행 선택 또는 재생 버튼으로 독립적인 결과 검토 플레이어 열기
- 완료 검토 플레이어에서 원본 영상과 선택 언어 번역 자막·원문 이중 자막 재생
- 검토 플레이어 오른쪽에 시간순 원문·번역 전체 목록과 미번역 구간 경고 표시
- 원문·번역 문장을 선택해 해당 영상 구간으로 즉시 이동하고 현재 구간 강조
- 검토 중 번역 언어와 원문 동시 표시를 전환하며 결과 개수 확인
