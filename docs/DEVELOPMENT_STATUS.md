...
- 위·아래 방향키 음량 조절 폭을 1%·2%·5%·10%로 설정하고 모든 플레이어에 즉시 적용
- 마지막 영상과 재생 위치 복원
- 여러 플레이어 창 지원
- 전체 화면 영화관 보기
...
- 번역 자막과 같은 구간의 원문 STT를 동시에 표시하는 이중 자막
- 원문 보조 자막의 반투명·이탤릭 스타일을 각각 설정하고 즉시 반영
- 번역이 없는 구간에서는 원문을 중복 없이 단일 자막으로 표시
- 자막 위치와 색상 저장
...
- 폴더 선택 후 모든 하위 폴더에서 지원 영상 재귀 검색
- 폴더 드래그앤드롭 시에도 하위 영상 자동 검색
- 대용량 폴더 비동기 검색, 검색 상태 표시 및 취소
- 폴더 검색 결과에서 대소문자·발음기호 차이를 무시한 동일 파일명 자동 검사
- 동일 이름 영상에 중복 개수 배지를 표시하고 전체 경로별 검토 창 제공
- 그룹마다 첫 영상을 유지 대상으로 두고 나머지를 목록 제외 대상으로 기본 선택
- 검토 선택을 바꿔 여러 중복 영상을 한 번에 대량 번역 목록에서 제거
- 중복 정리는 실제 영상 파일과 저장된 STT·번역 결과를 삭제하지 않음
- 숨김 파일, 앱 패키지 내부, 심볼릭 링크 건너뛰기
- 드래그 중 드롭 영역 강조와 지원 형식 안내
- 영상이 아닌 파일 및 이미 추가된 파일 자동 제외
- 메인 창의 현재 STT 모델, 번역 LLM, 원어, 대상 언어, 품질 설정 사용
- 현재 STT 설정과 번역 모델·대상 언어를 기준으로 기존 처리 결과 자동 확인
- 파일별 `미처리`, `STT 있음`, `일부 번역`, `번역 완료` 상태 표시
- 번역 완료 영상은 실행 대상에서 자동 제외하고 일부 완료 영상은 남은 단계부터 재개
- 설정 변경이나 외부 결과 갱신 후 사용할 수 있는 `기존 번역 확인` 수동 새로고침
...
- 목록의 영상 행 또는 상세 보기 버튼으로 독립적인 상세 진행 창 열기
- 상세 창에서 전체·STT·번역 진행률, 처리 단계, 현재 청크와 작업 메시지 실시간 갱신
- 상세 창에서 현재 생성 중인 내용과 최근 완료된 STT·LLM 번역 전문을 나란히 확인·복사
- 상세 창을 닫아도 백그라운드 작업을 유지하고 실패·취소 항목은 저장 결과부터 다시 시도
- 목록에서 단일·다중 영상 선택(⌘-클릭·Shift-클릭)
- 선택한 영상만 번역 시작·재개하거나 개별 중단
- 실행 중인 선택 항목 삭제 시 확인 후 해당 XPC 작업만 안전하게 취소
- 선택 항목을 실행 중에도 목록에서 삭제하며 저장된 STT·번역 중간 결과 유지
